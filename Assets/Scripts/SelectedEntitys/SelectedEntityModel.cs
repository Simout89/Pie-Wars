...
public class SelectedEntitysModel : MonoBehaviour
{
    ...
    public void SetNewSelectedUnitList(List<IEntity> unitList){            //при выделение новых entity
        this.ClearSelectedUnits();
        this._selectedEntitys = new(unitList);
        _historySelectedData.NewRecordInHistory(_selectedEntitys);
    }

     public void SetOldSelectedUnitList(List<IEntity> entityList){            //при выборе entity, которые были выделены ранее
        this.ClearSelectedUnits();
